...
<div class="container-fluid pl-5 pr-5">
  ...
  <div class="row">
    ...
    <li class="list-group-item col-12 text-center">
      <h3>All Listings (Newest First)</h3>
    </li>
      ...
      <li class="list-group-item col-2">
        <div class="row">
          ...
          <div class="col-12">
            <p><a href="/profile/{{ $listing->user->username }}">{{ $listing->user->username }}</a> wants to {{ $listing->type }} {{ $listing->quantity }}x <a href="/item/{{ $listing->item->id }}">{{ $listing->item->name }}</a> for {{ number_format($listing->price) }} {{ $listing->currency }} each</p>
          </div>
          ...
          <div class="col-12">
            ...
            <button class="btn btn-primary" data-name="{{ $listing->item->name }}" data-image="{{ $listing->item->image }}" data-price="{{ $listing->price }}" data-item_id="{{ $listing->item->id }}" data-lister="{{ $listing->user->id }}"
                    data-currency="{{ $listing->currency }}" data-quantity="{{ $listing->quantity }}" data-toggle="modal" data-target="#OfferModal" style="width:100%;">
